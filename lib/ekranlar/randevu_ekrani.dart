...
class _RandevuEkraniState extends State<RandevuEkrani> {
  ...
  final _neredenController = TextEditingController(); // [YENİ] Taksi için
  final _nereyeController = TextEditingController();  // [YENİ] Taksi için
  ...
  void initState() {
    ...
    _neredenController.addListener(() { if (mounted) setState(() {}); }); // [YENİ] Buton aktifliği için
    _nereyeController.addListener(() { if (mounted) setState(() {}); });  // [YENİ] Buton aktifliği için
  ...
  int _getToplamSure(List<Map<String, dynamic>> hizmetler, EsnafModeli esnaf) {
    ...
    // [YENİ] Taksi kategorisi için sabit süre (rezervasyon süresi mantığı)
    if (esnaf.kategori == 'Taksi' && toplam == 0) {
      return 30; // Varsayılan 30 dk
    }
  ...
  List<String> _slotlariUret(
    ...
    try {
      ...
      // [KESİN TAKVİM MANTIĞI]: Gece yarısını geçen dükkanlar için
      if (kapanisDk < acilisDk || is724) {
        // 1. Dünün devamı olan gece yarısı slotları (00:00'dan Kapanış'a kadar)
        // Bu slotlar takvimde 'Bugün'ün sabahıdır
        int currentDk = 0;
        ...
        // 2. Bugünün asıl mesai slotları (Açılış'tan Gece Yarısı'na kadar)
        currentDk = acilisDk;
        ...
        // Normal mesai saatleri (Örn: 09:00 - 22:00)
  ...
  String? _saatNedenKapali(
    ...
    for (var r in randevular) {
      ...
      // [YENİ]: Gece Mesaisi Çakışma Kontrolü Zekası
      String acilis = calisma['acilis'] ?? "09:00";
      int acilisDk = _saatiDakikayaCevir(acilis);
  ...
  Future<void> _randevuKaydet(EsnafModeli esnaf, Map<String, dynamic>? ajanda) async {
    ...
    String hAdi = "";
    if (_isAracKiralama) {
      hAdi = "${toplamSure > 1440 ? 'Günlük' : 'Saatlik'} Kiralama";
    } else if (isTaksi) {
                      ...
                      ValueListenableBuilder<String?>(
                        ...
                        builder: (context, saat, _) {
                          return SliverMainAxisGroup(
                            slivers: [
                              SliverToBoxAdapter(child: _adimBasligi("3", "Araç ve Sürücü Seçimi")),
                              if (saat == null) 
                                const SliverToBoxAdapter(child: Center(child: Padding(padding: EdgeInsets.all(20), child: Text("Lütfen önce saat seçiniz.", style: TextStyle(color: Colors.grey))))),
                              if (saat != null)
                            ...
                            return SliverToBoxAdapter(
                              child: Column(
                                children: [
                                  ...
                                  _adimBasligi(_isAracKiralama ? "3" : (esnaf.kategori == 'Taksi' ? "4" : "5"), "İletişim Bilgileri"),
                                  Container(
                                                  ...
                                                  Padding(
                                                    ...
                                                    child: ValueListenableBuilder<bool>(
                                                      ...
                                                      builder: (context, kendimSectim, _) {
                                                        ...
                                                        // [YENİ] Profesyonel Gece Yarısı Metni
                                                        ...
                                                        if (isSelected && !(_isAracKiralama) && widget.esnaf.kategori == 'Halı Saha') {
                                                           ...
                                                           if (sDk >= 0 && sDk < 360) {
                                                              ...
                                                              gosterimMetni = "Randevunuz $formatOnce'yi $formatBugun'e bağlayan gece gerçekleşecektir.";
                                                           }
                                                        }
  ...
  Widget _kanalSeciciWidget(EsnafModeli esnaf) {
    ...
    return ValueListenableBuilder<String?>(
      ...
      builder: (context, seciliKanal, _) {
        ...
        // [TAKSİ ÖZEL] Nöbet Çizelgesi ve Saat Filtreleme
        if (isTaksi && aracModu) {
          ...
          if (seciliTarih != null && seciliSaat != null) {
            ...
            liste = liste.where((arac) {
              ...
              String durum = (gunlukVeri?[plaka] ?? "C").toString(); // Varsayılan Çalışan (English C)

              if (durum.contains('I')) return false; // İstirahatli her zaman gizli
              
              if (isGunduz && durum.contains('C')) return true; // Gündüz çalışanlar
              if (isNobet && durum.contains('N')) return true;  // Nöbetçi çalışanlar

              // Eğer saat her iki dilime de girmiyorsa (mesai dışı) veya durum o saate uymuyorsa gizle
              return false;
            }).toList();
          } else if (seciliSaat == null) {
            // Saat seçilmediyse boş liste dönsün (Hata mesajı zaten build'de var)
            liste = [];
          }
        }
  ...
  Widget _saatSecimiBolumuSliver(EsnafModeli esnaf) {
    ...
    // [YENİ] Slotları Gruplandır (Gece vs Gündüz)
      ...
      builder: (context, aramaYapiliyor, _) {
        ...
        return ValueListenableBuilder<String?>(
          ...
          builder: (context, seciliSaat, _) {
            return SliverMainAxisGroup(
              slivers: [
                // 1. GECE YARISI BÖLÜMÜ
                if (geceSlotlari.isNotEmpty && esnaf.kategori == 'Halı Saha') ...[
                  SliverToBoxAdapter(
                    child: ValueListenableBuilder<DateTime?>(
                      ...
                      builder: (context, tarih, _) {
                        ...
                        return _vakitBasligi(
                          ...
                          "Gece Yarısı: $formatOnce'yi $formatBugun'e bağlayan gece", 
                ...
                // 2. GÜNDÜZ & AKŞAM BÖLÜMÜ
                if (gunduzSlotlari.isNotEmpty) ...[
                  SliverToBoxAdapter(
                    child: _vakitBasligi(
                      Icons.wb_sunny_rounded, 
                      "Gündüz & Akşam Seansları", 
  ...
  Widget _saatGridiUret(EsnafModeli esnaf, List<String> list, String? seciliSaat, int toplamSure) {
    return SliverGrid(
      gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
          ...
          childAspectRatio: 1.8, // [GÜNCELLEME] Dikey alan için oran biraz artırıldı
          mainAxisSpacing: 10,
          crossAxisSpacing: 10),
        ...
        return InkWell(
          ...
          child: Container(
            decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(8)), // Köşeler biraz daha yumuşatıldı
            child: Center(
              child: Column( // [İYİLEŞTİRME]: Row yerine Column ile alt alta dizilerek sığdırıldı
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  if (geceMi) ...[
                    Row(
                  ...
                  Text(slotMetni,
                      style: TextStyle(
                          ...
                          fontSize: seciliSaat == slot ? 14 : 12, // Sığması için fontlar milimetrik optimize edildi
  ...
  // [YENİ] Taksi İçin Özel UI Metotları
                ...
                Expanded(
                  child: InkWell(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        const Text("TARİH", style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Colors.grey, letterSpacing: 1.2)),
                        ...
                        Row(
                          children: [
                            ...
                            Text(
                              tarih == null ? "Seçiniz" : DateFormat('dd MMMM', 'tr_TR').format(tarih),

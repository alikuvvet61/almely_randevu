...
class _EsnafDetayEkraniState extends State<EsnafDetayEkrani> {
  ...
  bool _zamanAraligindaMi(String? baslangic, String? bitis) {
    if (baslangic == null || bitis == null) return false;
    if (baslangic == "00:00" && bitis == "00:00") return true; // 7/24 veya tanımlanmamış

    final simdi = DateTime.now();
    final simdiDakika = simdi.hour * 60 + simdi.minute;

    final basParcalar = baslangic.split(':');
    final bitParcalar = bitis.split(':');

    if (basParcalar.length != 2 || bitParcalar.length != 2) return false;

    int basDakika = int.parse(basParcalar[0]) * 60 + int.parse(basParcalar[1]);
    int bitDakika = int.parse(bitParcalar[0]) * 60 + int.parse(bitParcalar[1]);

    if (bitDakika <= basDakika) {
      // Gece yarısını aşan saatler (örn 22:00 - 06:00)
  ...
  Widget build(BuildContext context) {
    return Scaffold(
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(_guncelEsnaf.kategori == 'Taksi' ? "Gündüz Mesai Saatleri" : "Çalışma Programı ve Hizmet Bilgisi", style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                  const SizedBox(height: 10),
                  Container(
                        ...
                        SizedBox(
                          ...
                          child: Builder(builder: (context) {
                            ...
                            final duraktakiAraclar = tumAraclar.where((a) {
                              bool istirahatli = _istirahatKontrol(a);
                              if (istirahatli) return false;

                              // [YENİ] Taksi için Saat Bazlı Görünürlük
                              if (_guncelEsnaf.kategori == 'Taksi') {
                                String plaka = a['plaka'] ?? "";
                                String durum = (_gunlukAjanda[plaka] ?? "C").toString();
                                
                                String mesaiAcilis = _guncelEsnaf.calismaSaatleri?['acilis'] ?? "08:00";
                                String mesaiKapanis = _guncelEsnaf.calismaSaatleri?['kapanis'] ?? "20:00";
                                String nobetBas = _guncelEsnaf.nobetBaslangic ?? "20:00";
                                String nobetBit = _guncelEsnaf.nobetBitis ?? "08:00";
                                
                                bool inDayRange = _zamanAraligindaMi(mesaiAcilis, mesaiKapanis);
                                bool inNightRange = _zamanAraligindaMi(nobetBas, nobetBit);

                                if (inDayRange && durum.contains('C')) return true;
                                if (inNightRange && durum.contains('N')) return true;
                            ...
                            // [YENİ] Ana listeyi de aynı mantıkla filtrele (Taksi ise)
                            if (_guncelEsnaf.kategori == 'Taksi') {
                              tumAraclar = tumAraclar.where((a) {
                                if (_istirahatKontrol(a)) return false;
                                String plaka = a['plaka'] ?? "";
                                String durum = (_gunlukAjanda[plaka] ?? "C").toString();
            ...
            if (!_guncelEsnaf.randevuAlinmasin)
              AnaButon(
                ...
                onPressed: () {
                  ...
                  // [TAKSİ ÖZEL] Taksi kategorisinde manuel ajanda yerine nöbet çizelgesi kullanıldığı için her zaman var sayılır.
                  if (!_guncelEsnaf.ajandayiKendimAyarlayacagim || _guncelEsnaf.kategori == 'Taksi') {
                    ajandaVarMi = true;
                  } else {
      ...
      floatingActionButton: _guncelEsnaf.kategori == 'Taksi' 
        ? SosButonu(
            randevuId: "genel_sayfa", 
            esnafId: _guncelEsnaf.id, 
            kullaniciTel: widget.kullaniciTel ?? "", 
            baslatanRol: 'Yolcu', 
            adminTel: _guncelEsnaf.telefon
          )
        : null,
    );
  }

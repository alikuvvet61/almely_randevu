...
import 'kira_teslimat_ekrani.dart';

class AracKiralamaEsnafPaneli extends StatefulWidget {
  final EsnafModeli esnaf;
  final String? soforTel;
  final bool openFilo;
  final bool openMesai;

  const AracKiralamaEsnafPaneli({super.key, required this.esnaf, this.soforTel, this.openFilo = false, this.openMesai = false});

  @override
  State<AracKiralamaEsnafPaneli> createState() => _AracKiralamaEsnafPaneliState();
}

class _AracKiralamaEsnafPaneliState extends State<AracKiralamaEsnafPaneli> {
      ...
      children: [
        ...
        if (!_guncelEsnaf.randevuAlinmasin)
          _yonetimKarti(
            ...
            renk: Colors.indigo,
            onTap: () => Navigator.push(context, MaterialPageRoute(builder: (c) => AracKiralamaEsnafRandevuOnayEkrani(esnafId: widget.esnaf.id, esnaf: widget.esnaf))),
          ),
        _yonetimKarti(
          ...
          onTap: () async {
            final navigator = Navigator.of(context);
            await navigator.push(MaterialPageRoute(builder: (c) => AracParametreEkrani(esnafId: widget.esnaf.id)));
            _verileriTazele();
          },
        ),
        _yonetimKarti(
          ...
          _yonetimKarti(
            ...
            onTap: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (c) => AracKiralamaRandevuEkrani(
                    esnaf: _guncelEsnaf,
                    kullaniciTel: null,
                  ),
                ),
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            "${DateFormat('HH:mm').format(iadeZamani.subtract(Duration(minutes: r.uzatmaSuresi)))} ile ${DateFormat('HH:mm').format(iadeZamani)} arası",
                            style: TextStyle(
            ...
            Center(
              child: Container(
                ...
                decoration: BoxDecoration(
                    ...
                    borderRadius: BorderRadius.circular(10)),
                child: Text(rozetMetni,
                    style: TextStyle(
                        color: temaRengi, fontWeight: FontWeight.bold)),
              ),
            ),
          ],
        ),
            ...
            return SingleChildScrollView(
              ...
              child: Column(
                children: [
                  ...
                  // [YENİ] ÇAKIŞMA YÖNETİMİ: KRİTİK ALARMLAR
                  if (widget.esnaf.kategori == 'Araç Kiralama')
                    _cakismaAlarmlari(hepsi),
                    ...
                    Padding(
                      ...
                      child: InkWell(
                        onTap: () => Navigator.push(
                          context,
                          MaterialPageRoute(builder: (c) => AracKiralamaEsnafRandevuOnayEkrani(esnafId: widget.esnaf.id, esnaf: widget.esnaf))
                        ),
                        child: Container(
                          padding: const EdgeInsets.all(16),
                          decoration: BoxDecoration(
  ...
  void _muadilAracaKaydir(RandevuModeli r, List<RandevuModeli> hepsi) async {
    ...
    for (var k in kiralikAraclar) {
      ...
      for (var checkR in hepsi) {
        ...
        if (rBas.isBefore(cBit) && rBit.isAfter(cBas)) {
          dolu = true; break;
        }
      }
